...
public class CreateInventoryUseCase
{
    ...
    public async Task<InventoryItem?> ExecuteAsync(Guid productId, Guid warehouseId, int quantity, string? imageKey, CancellationToken cancellationToken = default)
    {
        ...
        var item = InventoryItem.Create(productId, warehouseId, new StockQuantity(quantity), imageKey);
