...
public class AuthController : ControllerBase
{
    ...
    /// Bu token'ı Ordering.API ve Inventory.API isteklerinde Authorization: Bearer &lt;access_token&gt; olarak kullanın.
    /// </summary>
    [HttpPost("login")]
    ...
    /// <summary>
    /// Keycloak'ta kullanıcı oluşturur (user role atanır) ve Ordering.API'de Customer kaydı açar.
    /// </summary>
    [HttpPost("register")]
    ...
    public async Task<IActionResult> Register([FromBody] RegisterRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var orderApiBase = _configuration["Ordering.API:BaseUrl"]?.TrimEnd('/') ?? "https://localhost:5001";
        try
        {
            using var client = _httpClientFactory.CreateClient();
            ...
            if (!response.IsSuccessStatusCode)
                _logger.LogWarning("Keycloak user created but Ordering.API Customer failed: {Status}", response.StatusCode);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Ordering.API Customer oluşturulamadı");
        }
